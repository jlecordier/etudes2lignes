...
import {
  elementImagePleineLargeur,
  revoquerLesUrls,
} from '../../trajets/ui/elementsDImage';
import { texteDEtatDuSuivi } from '../domain/presentation';
import { calculerCibleDeScroll, calculerDefilement, type EtapeDuVoyage } from '../domain/projection';
...
export function creerSuiviScreen(dependances: DependancesSuivi): {
  ...
  function rendreLaPile(): void {
    ...
    pile.replaceChildren(
      ...(trajet?.images ?? []).map((image) => elementImagePleineLargeur(image, urlsARevoquer)),
    );
  }
