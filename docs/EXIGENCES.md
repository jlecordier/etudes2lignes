...
| GR-3 | Ajouter un point (image → carte) apparaît en liste et en marker              | `E e2e/points.spec.ts`                              |
| GR-4 | Clic droit sur l'image place un point directement puis choisit la coordonnée | `E e2e/points.spec.ts`                              |
| GR-5 | Déplacer un point sur l'image / sur la carte (liste ou button flottant)      | `E e2e/points.spec.ts`                              |
...
| GR-8 | Carte intégrée : markers numérotés, déplaçables ; placement responsive       | `E e2e/carte-editeur.spec.ts`                       |
...
| SU-2  | Position sur une étape → target = offset de l'étape ; entre deux → interpolation | `U projection.test.ts`                                          |
| SU-3  | Avant/après le trajet → target bornée à la première/dernière étape               | `U projection.test.ts`                                          |
| SU-4  | Seuil « hors trajet » adaptatif `max(5 km, 20 % du segment)`                     | `U projection.test.ts`                                          |
| SU-5  | Segment de length nulle (jonction) → pas de division par zéro                    | `U projection.test.ts`                                          |
| SU-6  | Adhérence : le bruit GPS près d'une jonction ne fait pas sauter la page          | `U projection.test.ts`                                          |
| SU-7  | La target est placée aux trois quarts hauts, bornée au document                  | `U projection.test.ts` (`computeScroll`)                        |
...
| SU-11 | Erreurs passagères tolérées tant que le last fix est frais (tunnels)             | `U GeolocationPositionSource.test.ts`                           |
...
| #    | Exigence                                                                    | Vérifié par                                           |
| ---- | --------------------------------------------------------------------------- | ----------------------------------------------------- |
| IE-1 | Export → file JSON autonome (nom, images base64, points par index)          | `U trajetJson.test.ts`, `E e2e/import-export.spec.ts` |
...
| #    | Exigence                                                                                                                                                                                                                       | Vérifié par                                                                                         |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| QA-1 | Le code typecheck sans error                                                                                                                                                                                                   | `pnpm typecheck` (CI)                                                                               |
| QA-2 | Lint type-aware strict à zéro (aucun `!`, aucun `as` de forme — les casts de **marque** d'identifier restent tolérés dans `domain/ids.ts` et à la frontière de persistance, cf. [ADR 0005](adr/0005-indexeddb-arraybuffer.md)) | `pnpm lint` (CI) — cf. [ADR 0002](adr/0002-lint-type-aware-strict.md)                               |
...
| QA-4 | Les garde-fous du domaine et des adapters ont un témoin exécutable — hormis les mutants équivalents et les gardes inatteignables, commentés sur place                                                                          | `pnpm mutation` (à la request, **hors CI**) — cf. [ADR 0006](adr/0006-tests-de-mutation-stryker.md) |
