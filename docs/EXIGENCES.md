...
| GR-5  | Déplacer un point sur l'image / sur la carte, depuis les boutons de son repère                                                                   | `E e2e/points.spec.ts`                                                                                                                                                   |
...
| GR-17 | Les deux cartes montrent la position qui pilote l'appli : le GPS réel, ou la position simulée quand la simulation est active                     | `U SuiviScreen.test.ts`, `U TrajetEditorScreen.test.ts`, `E e2e/gps.spec.ts`                                                                                             |
| GR-18 | Une position trop imprécise pour caler la page s'affiche quand même, cerclée de son incertitude mesurée                                          | `U GeolocationPositionSource.test.ts`, `U LeafletCarteDesPoints.test.ts`, `U LeafletCoordonneeSelector.test.ts`, `U SuiviScreen.test.ts`, `U TrajetEditorScreen.test.ts` |
| GR-19 | Sans position, aucun marqueur — et la phrase qui dit pourquoi s'affiche à côté de la carte                                                       | `U TrajetEditorScreen.test.ts`, `U LeafletCoordonneeSelector.test.ts`, `U LeafletCarteDesPoints.test.ts`                                                                 |
| GR-20 | Le cadrage englobe la position quand elle est déjà connue, et ne se refait jamais à l'arrivée d'une position                                     | `U fitting.test.ts`, `U LeafletCarteDesPoints.test.ts`, `U LeafletCoordonneeSelector.test.ts`                                                                            |
| GR-21 | « Ma position » amène la carte sur elle, au zoom d'un point unique, et reste inerte tant qu'elle est inconnue                                    | `U TrajetEditorScreen.test.ts`, `U LeafletCoordonneeSelector.test.ts`, `E e2e/gps.spec.ts`                                                                               |
| GR-22 | Une seule session de position par écran de carte : l'éditeur en ouvre une à l'arrivée — sa carte est toujours en page — et la referme en partant | `U TrajetEditorScreen.test.ts`                                                                                                                                           |
...
| GR-24 | Zoom et sélection natifs neutralisés dans toute l'application ; les champs de saisie restent sélectionnables ; la carte garde son pincement      | `U pinchZoom.test.ts`, `E e2e/gestes.spec.ts`                                                                                                                            |
| GR-25 | Un appui long sur l'image nue arme un repère fantôme, que le doigt déplace de page en page jusqu'au relâchement, où le point se crée             | `U addPointOnStack.test.ts`, `E e2e/points.spec.ts`                                                                                                                      |
| GR-26 | Un tap à deux doigts sur une même page crée un point au milieu de leur enveloppe, sans attendre l'appui long                                     | `U addPointOnStack.test.ts`                                                                                                                                              |
| GR-27 | Un seul geste ne crée jamais plus d'un point, `contextmenu` natif compris                                                                        | `U addPointOnStack.test.ts`, `E e2e/points.spec.ts`                                                                                                                      |
